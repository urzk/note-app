...
export const EditorWrapper = () => {
  const [editorViewState, setEditorViewState] = useAtom(editorViewStateAtom);
    ...
    <div className="flex flex-col flex-1">
      <div className={"flex w-full h-screen overflow-auto"}>
        <div className="flex-1 view-wrapper">
          {editorViewState === "milkdown" && <CrepeEditor />}
          {editorViewState === "editor" && <CodeMirror />}
          <div
            className="absolute top-0 right-0 opacity-75"
            onClick={() => {
              setEditorViewState(
                editorViewState === "milkdown" ? "editor" : "milkdown",
              );
            }}
          >
